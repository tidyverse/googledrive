...
- Make sure the package still passes `R CMD check` locally for you. It’s
  a good idea to do that before you touch anything, so you have a
  baseline.
...
- Tests: please *try* to run our tests or at least those that exercise
  ...
  auth-requiring packages like googledrive and are open to constructive
  feedback. More below.
...
- If the PR is related to an issue, link to it in the description, with
  ...
  make sure one of the commit messages includes [text like `closes #44`
  or
  ...
  issue is mentioned in the title, because auto-linking does not work in
  the PR title.
  - GOOD PR title: “Obtain user’s intent via mind-reading; fixes \#86”.
  - BAD PR title: “Fixes \#1043”. Please remind us all what issue \#1043
    is about!
  - BAD PR title: “Something about \#345”. This will not actually close
    issue \#345 upon merging. [Use the magic
...
- Add a bullet to `NEWS.md` with a concise description of the change, if
  ...
  [dplyr’s
  `NEWS.md`](https://github.com/tidyverse/dplyr/blob/main/NEWS.md) is a
  ...
  any organization into sub-sections just prior to a release. What
  merits a bullet?
...
- Be pipe-friendly.
- If it’s not well-documented (e.g. working example!), it doesn’t really
  exist.
...
A few hand-picked functions support passing extra parameters through to
the API request via `...`, but we don’t do this across the board.
...
These functions are exported for use by programming-oriented users who
are willing to read [Drive API
...
- This reflects the approach documented in the gargle vignette [Managing
  tokens
...
- If you want to use a token you already have, edit those lines to use
  ...
  use it as a pragmatic way to run tests with a token you have on hand.
...
1.  Source `tests/testthat/driver.R` to extract and aggregate the
    ...
    - This creates two R scripts: `tests/testthat/all-test-setup.R` and
...
to truly delete the test files.
...
- Test files are marked up with knitr chunk headers in comments,
  e.g. `# ---- clean ----` or `# ---- tests ----`. This is what enables
  the `driver.R` script to isolate the setup or cleaning code. Don’t
  break that.
...
- All test files should have a name that documents why they exist and
  ...
  Always use one of these functions to generate file names. Use `nm_()`
  for test files that persist. Use `me_()` for ephemeral test files that
  are created and destroyed in one test run.
...
Code of
Conduct](https://googledrive.tidyverse.org/dev/CODE_OF_CONDUCT.md). By
