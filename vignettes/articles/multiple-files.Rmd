...
### Upload all files into a folder called "to-upload"

```{r, eval = FALSE}
library(purrr)
```

Let's grab a set of files to upload.

```{r, eval = FALSE}
local_files <- list.files(file.path(R.home("doc"), "html"), full.names = TRUE)
```
...
```{r, eval = FALSE}
new_names <- glue::glue("{Sys.Date()}_{basename(local_files)}")
