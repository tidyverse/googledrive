...
The `shared` column shows that this file is not yet shared with anyone and, for those so inclined, detailed information on permissions can be found in the `permissions_resource` list-column. The `drive_resource`, which is always present in a dribble, typically also contains information on permissions.
...
file <- file %>%
  drive_share(
    role = "writer",
    type = "user",
    emailAddress = "jenny@rstudio.com",
    emailMessage = "Would appreciate your feedback on this!"
  )
file
```

We see that the file is now shared. More details about "who has what role" on this file can be excavated from the `permissions_resource`.

Let's say we also want anyone to be able to read the file.
