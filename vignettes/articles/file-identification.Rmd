...
You can get similar results with `drive_find()`. By default, it gets info on the most recent 100 files.
...
```{r}
## just folders
drive_find(q = "mimeType = 'application/vnd.google-apps.folder'") %>% head(3)

## just Sheets
drive_find(q = "mimeType='application/vnd.google-apps.spreadsheet'") %>% head(3)
```

Files containing `'horsebean'` in their content:

```{r}
drive_find(q = "fullText contains 'horsebean'") %>% head(3)
```

Files in your "Shared with me" collection

```{r}
drive_find(q = "sharedWithMe = true") %>% head(3)
```
...
drive_find(q = "name contains 'wts'")

## therefore we do regex matching on the R side
drive_find(pattern = "wts")
```

If you know the exact name, you can use `drive_find()` or, less cumbersome, the `drive_path()` function described next.

```{r}
drive_find(q = "name = '538-star-wars-survey'")
```
