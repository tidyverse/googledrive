...
test_that("as_id() properly returns character strings as ids", {

  expect_s3_class(as_id("12345"), "drive_id")

  expect_equal(as_id("12345")[1], "12345")

  expect_equal(as_id(character(0)), character(0))
})

test_that("as_id() is properly recognizes URLs", {
  x <- c(
  ...
  expect_equal(as_id(x), as_id(c("doc12345", NA, "blob12345")))

  ## properly recognizes a non-conforming URL
  x <- "http://example.com"
  expect_equal(as_id(x)[1], NA_character_)
})
