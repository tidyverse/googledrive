...
#' @examples
#' \dontrun{
#' teamdrive_get("my-awesome-team-drive")
#' teamdrive_get(c("apple", "orange", "banana"))
#' teamdrive_get(as_id("KCmiHLXUk9PVA-0AJNG"))
#' teamdrive_get(as_id("https://drive.google.com/drive/u/0/folders/KCmiHLXUk9PVA-0AJNG"))
#' teamdrive_get(id = "KCmiHLXUk9PVA-0AJNG")
#' teamdrive_get(id = "https://drive.google.com/drive/u/0/folders/KCmiHLXUk9PVA-0AJNG")
#' }
teamdrive_get <- function(name = NULL, id = NULL, verbose = TRUE) {
  ...
  if (!is.null(name)) {
    ...
    return(teamdrive_from_name(name))
  }

  stopifnot(is.character(id))
  as_dribble(purrr::map(as_id(id), get_one_teamdrive_id))
}

get_one_teamdrive_id <- function(id) {
...
teamdrive_from_name <- function(name = NULL) {
  if (length(name) == 0) return(dribble())

  teamdrives <- teamdrive_find(verbose = FALSE)
  if (no_file(teamdrives)) return(dribble())

  teamdrives <- teamdrives[teamdrives$name %in% name, ]
  ## TO DO: if (verbose), message if a name matches 0 or multiple Team Drives?

  teamdrives[order(match(teamdrives$name, name)), ]
}
