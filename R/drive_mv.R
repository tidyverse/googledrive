...
#' @export
drive_mv <- function(file = NULL, folder = NULL, verbose = TRUE){

  request <- build_drive_mv(file = file, folder = folder)
  response <- make_request(request)
  proc_res <- process_drive_mv(response = response, file = file, folder = folder, verbose = verbose)

  file <- drive_file(proc_res$id)
  invisible(file)
}

build_drive_mv <- function(file = NULL, folder = NULL, token = drive_token()){
  ...
  url <- file.path(.state$drive_base_url_files_v3,
...
process_drive_mv <- function(response = NULL, file = NULL, folder = NULL, verbose = TRUE){
